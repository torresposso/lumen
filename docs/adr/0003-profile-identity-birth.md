...
the existing profile (the second `add`'s `name`/`city` are discarded). The
`id` is an auto-generated opaque UUID, and `get`/`rm` accept a UUID only —
there is no lookup by name or city anywhere.
...
- `add` of an already-known birth is effectively idempotent: the caller gets
  ...
  pre-flight.
- The `name`/`city` fields are display metadata only — never identity, never
  a lookup key. This is why the model carries them off the uniqueness path
  (they are discarded on conflict).