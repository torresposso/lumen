...
describe("chartCommand — lumen chart natal <name>", () => {
	...
	test("returns full chart output for a valid profile", async () => {
		...
		const result = await chartCommand(["natal", profile.name], {
			profiles: store,
			ephemeris,
		});
		...
		const chart = (
			result as {
				chart: {
					meta: { houseSystem: string; zodiac: string };
					bodies: { pluto?: { sign: string } };
				};
			}
		).chart;
	...
	test("throws NOT_FOUND for a nonexistent profile name", async () => {
		const store = new InMemoryProfileStore(new Database(":memory:"));
		const nonexistentName = "ghost";

		let error: unknown;
		try {
			await chartCommand(["natal", nonexistentName], {
				profiles: store,
				ephemeris,
			});
		} catch (e) {
		...
		expect(axiErr.message).toContain(nonexistentName);
		expect(axiErr.suggestions[0]).toContain("lumen profile list");
	});

	test("throws VALIDATION_ERROR when missing the profile name argument", async () => {
		const store = new InMemoryProfileStore(new Database(":memory:"));
	...
	test("returns natalInterpretation when --interpret flag is passed", async () => {
		...
		const result = (await chartCommand(["natal", profile.name, "--interpret"], {
			profiles: store,
			ephemeris,
		})) as {
			natalInterpretation: {
	...
	test("returns usage with --help", async () => {
		...
		expect(usage as string).toContain("lumen chart natal <name>");
